...
export const getTextWidth = (text: string, font: any) => {
...
export const getTextWidthList = (textList: string[] | number[], font: any) => {
  const canvas = document.createElement('canvas')
  const context = canvas.getContext('2d')!
  context.font = font
  const widthList = textList.map(item => {
    return context.measureText(String(item)).width
  })
  // var metrics = context.measureText(text)
  return widthList
}